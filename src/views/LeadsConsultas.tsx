...
const ConsultasLeads = ({ initialLeads = [] }: ConsultasLeadsProps) => {
  ...
  const [tipoContacto, setTipoContacto] = useState<
    "presencial" | "whatsapp" | "web"
  >("whatsapp");
  const [filtro, setFiltro] = useState<
    "todos" | "pendiente" | "en_proceso" | "finalizado"
  >("todos");
  ...
  const { data: consultas = [], isLoading: cargandoConsultas } = useQuery<
    Lead[]
  >({
  ...
  const enProceso = consultas.filter(
    (lead) => lead.estado === "en_proceso",
  ).length;
            ...
            <div className="flex w-full mx-auto gap-8 p-4">
              ...
              <div className=" flex-1 flex flex-col  p-1 max-w-7xl mx-auto">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
                  <div>
                    ...
                    <p className="text-sm text-slate-500 mt-1">
                      Gestioná los interesados de tus propiedades publicadas y
                      hacé el seguimiento de contactos.
                    </p>
                  </div>
                ...
                <div className="flex border-b border-slate-200 gap-2">
                  {(
                    ["todos", "pendiente", "en_proceso", "finalizado"] as const
                  ).map((tipo) => (
                    <button
                      key={tipo}
                ...
                <div className="mt-12 bg-white rounded-2xl border border-slate-200/60 shadow-sm overflow-hidden">
                  {leadsFiltrados.length === 0 ? (
                    <div className=" p-12 text-center text-slate-500">
                      No se encontraron consultas en esta sección.
                    </div>
                  ) : (
                          ...
                          {nuevoLead.propiedadId &&
                            (() => {
                              const propSeleccionada = propiedades.find(
                                (p) =>
                                  p.id?.toString() === nuevoLead.propiedadId,
                              );
