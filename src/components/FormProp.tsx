...
const FormProp = () => {
  ...
  const currentBarrio = watch("barrio");
  const currentHabitaciones = watch("habitaciones");
  const currentWc = watch("wc");
  const currentCochera = watch("cochera");
  const currentPrecio = watch("precio");
  const currentExpensas = watch("expensas");
  ...
  const onSubmitForm = async (data: FormPropsData) => {
    try {
      ...
      const precioLimpio =
        parseInt(data.precio.replace(/[^0-9]/g, ""), 10) || 0;
      const expensasLimpias =
        parseInt(data.expensas.replace(/[^0-9]/g, ""), 10) || 0;
      ...
      if (res.data?.ok === true) {
        ...
        toast.error(res.data?.msg || "Ocurrió un error", {
          position: "bottom-right",
          duration: 1500,
        });
      }
                  ...
                  render={({ field, fieldState }) => (
                    <Field data-invalid={fieldState.invalid}>
                      ...
                      <Input
                        ...
                        value={field.value}
                        onChange={(e) =>
                          setValue("precio", formatCurrency(e.target.value))
                        }
                      />
                      {fieldState.invalid && (
                        <p className="text-sm text-red-500 mt-1">
                  ...
                  render={({ field, fieldState }) => (
                    <Field data-invalid={fieldState.invalid}>
                      ...
                      <Input
                        ...
                        value={field.value}
                        onChange={(e) =>
                          setValue("expensas", formatCurrency(e.target.value))
                        }
                      />
                      {fieldState.invalid && (
                        <p className="text-sm text-red-500 mt-1">
          ...
          <Field className="mt-8">
            ...
            <div className="flex flex-wrap gap-3 mb-3">
              {images.map((img, idx) => (
                <div
                  key={idx}
                  className="group relative h-40 w-40 overflow-hidden rounded-lg border border-zinc-200"
                >
                  <img
